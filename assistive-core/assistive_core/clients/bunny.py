"""Bunny.net media storage client.

Ported from beekeeper ``services/bunny_storage_service.py``. Config is read from
``assistive_core.settings`` rather than ``os.getenv`` inline. The uploader is
namespaced per-vertical via a ``folder`` prefix arg so multiple verticals can
share one storage zone without colliding.
"""
...
class BunnyStorageService:
    ...
    async def upload_photo(
        ...
        Args:
            ...
            folder: The folder (per-vertical prefix) to store the file in
                (default: ``"inspections"``).
    ...
    async def delete_photo(self, file_path: str) -> bool:
        ...
        Args:
            file_path: The path of the file to delete
                (e.g., ``"inspections/20240101_120000_photo.jpg"``).
